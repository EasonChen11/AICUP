...
def test_pred(X_train_scaled, y_train, scaler, le):
    # for test data
    # 若尚未產生特徵，請!先執行 data_generate() 生成特徵 CSV 檔案
    # data_generate(datapath='./test_data', tar_dir='tabular_data_test') # test data

    group_size = 27
    test_info = pd.read_csv('./test_info.csv') # only unique_id , mode and cut_point
    ...
    def predict_binary(X_train, y_train, X_test): # return predicted probability
    ...
    def predict_multiary(X_train, y_train, X_test): # return predicted probability
        clf = RandomForestClassifier(random_state=42)
        clf.fit(X_train, y_train)
        predicted = clf.predict_proba(X_test)
        num_groups = len(predicted) // group_size
    ...
    with open('test_pred.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['unique_id', 'gender', 'hold racket handed', 'play years_0', 'play years_1', 'play years_2', 'level_2', 'level_3', 'level_4', 'level_5'])
        ...
        for i in range(len(test_unique_id)):
            writer.writerow([test_unique_id[i], gender_pred[i], hold_pred[i], years_pred[i][0], years_pred[i][1], years_pred[i][2], level_pred[i][0], level_pred[i][1], level_pred[i][2], level_pred[i][3]])    
    

def main():
    # 若尚未產生特徵，請先執行 data_generate() 生成特徵 CSV 檔案
    # data_generate(datapath='./train_data', tar_dir='tabular_data_train') # train data
    ...
    test_pred(X_train_scaled, y_train, scaler, le)
